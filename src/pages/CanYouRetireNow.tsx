...
const CanYouRetireNow: React.FC = () => {
  ...
  const simulation = useMemo(() => {
    ...
    // If corpus is zero or expenses are not set, return empty but valid structure
    if (currentCorpus <= 0 || currentExpense <= 0) {
        return { projections: [], yearsLasted: 0, status: 'warning' };
    }
            ...
            <CardDescription className="text-foreground/80">
              {simulation.status === 'success' 
                ...
                : simulation.yearsLasted > 0 
                  ...
                  : "Please ensure your assets and annual expenses are recorded in the Assets and Retirement sections."}
            </CardDescription>
          </div>
        </CardHeader>
      </Card>
                ...
                {simulation.projections.length > 0 ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                      No projection data available. Check your assets and expenses.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
