...
const initialAssets: PlatinumAsset[] = [
  { id: '1', particulars: 'Platinum Jewellery', value: 0, grams: 0 },
  { id: '2', particulars: 'Platinum Bars', value: 0, grams: 0 },
  { id: '3', particulars: 'Platinum ETF', value: 0 },
];

const Platinum: React.FC = () => {
  const [assets, setAssets] = useState<PlatinumAsset[]>(initialAssets);
  const [platinumPrice, setPlatinumPrice] = useState<number>(0);

  useEffect(() => {
    try {
      const savedPrice = localStorage.getItem('platinumPrice');
      if (savedPrice) {
        setPlatinumPrice(JSON.parse(savedPrice));
      }

      const saved = localStorage.getItem('platinumData');
      if (saved) {
        let savedAssets = JSON.parse(saved) as PlatinumAsset[];
      ...
      console.error("Failed to load or migrate platinum data", e);
    }
  }, []);

  useEffect(() => {
    localStorage.setItem('platinumData', JSON.stringify(assets));
    localStorage.setItem('platinumPrice', JSON.stringify(platinumPrice));
  }, [assets, platinumPrice]);

  useEffect(() => {
    setAssets(prev =>
      prev.map(p => {
        if (p.particulars === 'Platinum Jewellery' || p.particulars === 'Platinum Bars') {
          return { ...p, value: (p.grams || 0) * platinumPrice };
        }
        return p;
      })
    );
  ...
  const handleGramsChange = (id: string, grams: number) => {
    setAssets(prev =>
      prev.map(p => (p.id === id ? { ...p, grams, value: grams * platinumPrice } : p))
    );
  };
  ...
  const exportData = () => {
    const dataToExport = { platinumPrice, assets };
    ...
    reader.onload = e => {
      try {
        ...
        if (data.assets && Array.isArray(data.assets)) {
          ...
          if (typeof data.platinumPrice === 'number') {
            setPlatinumPrice(data.platinumPrice);
          }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <Label htmlFor="platinum-price" className="whitespace-nowrap">Platinum Price/gram (₹)</Label>
            <Input
              id="platinum-price"
              type="text"
              value={platinumPrice.toLocaleString('en-IN')}
              onChange={(e) => setPlatinumPrice(Number(e.target.value.replace(/,/g, '')) || 0)}
              className="w-32"
            />
          </div>
        </div>
          ...
          <Card className="h-full border-0 shadow-none rounded-none">
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {assets.map(p => {
                    const isGramsBased = p.particulars === 'Platinum Jewellery' || p.particulars === 'Platinum Bars';
                    return (
